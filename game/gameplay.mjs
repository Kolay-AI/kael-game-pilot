...
export function kaelRenderFrame(pose,frame,face=0){let raw=[0,0,0,0],upperY=0,lean=0,bodyTilt=0,rotation=0,pelvisX=0,pelvisY=0;if(pose==='walk')raw=WALK_LEGS[frame%24];const directionalRun=(face===1||face===-1)&&pose==='sprint',rightRun=directionalRun;if(pose==='sprint'){raw=(rightRun?RIGHT_SPRINT_LEGS:SPRINT_LEGS)[frame%12];lean=5;bodyTilt=-2}if(rightRun){const i=frame%12;lean=pose==='sprint'?8:7;pelvisX=[0,3,6,7,4,0,-4,-7,-5,0,3,1][i];pelvisY=[0,0,2,4,2,0,0,2,3,1,0,0][i]}if(pose==='jumpUp')raw=[[-3,1,5,-3],[-2,0,4,-5],[1,-2,5,-6]][frame%3];if(pose==='flip'){raw=FLIP_META[Math.min(15,Math.max(0,frame))];rotation=(Math.min(15,Math.max(0,frame))/15)*Math.PI*2;upperY=-3}if(pose==='fall')raw=[[4,-3,-4,0],[6,-1,-6,2],[3,1,-2,4]][frame%3];if(pose==='melee'){const phase=frame<2?'windup':frame<5?'active':'recover',step=frame%5+1;raw=step===2?(phase==='active'?[-5,0,9,-5]:[-2,1,4,-1]):step===3?(phase==='active'?[5,-3,-4,1]:[2,0,-2,1]):step===5?(phase==='active'?[-2,2,3,2]:[0,1,1,1]):(phase==='active'?[3,-1,-2,1]:[0,0,0,0]);lean=phase==='active'?2:phase==='windup'?-1:0}if(pose==='idle')upperY=[0,-1,0,1,0,-1,0,1][frame%8];else if(pose==='walk')upperY=[1,0,-1,0,0,-1,0,1,0,-1,0,1][Math.floor((frame%24)/2)];else if(pose==='sprint')upperY=[1,0,-1,0,0,-1,0,1,0,-1,0,1][frame%12];if(pose==='crouch')upperY=17;if(pose==='takeoff')raw=frame<2?[-2,3,3,2]:[-3,0,4,-4];if(pose==='land')raw=frame<2?[2,-1,-2,0]:frame<4?[0,2,2,1]:[0,0,0,0];if(pose==='takeoff'||pose==='land')upperY=frame<2?5:2;if(pose==='hit')lean=-4;const floor=Math.max(raw[1],raw[3],0),legs=[raw[0],raw[1]-floor,raw[2],raw[3]-floor];
